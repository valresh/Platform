Enumeration_A(ARWNULTOP,"Anti-Reset Windup Multiple Output (OP)",8,Normal,Hi,Lo,HiLo,Def=Normal)//Status - Applies to function blocks with multiple output (OP) connections. 
//Contains the windup status for each output variable.
...
Real_A(CV,"Calculated Value",8,NaN)//The result (calculated value) of the function blocks. The value may be in Percent or Engineering Units, depending on the function block
...
Integer(NUMSEC,"",0)
Real_A(OP,"in Percent",8,0)
Real_A(OPBIAs,"Total Output Bias (in Engineering Units)",8,0)//is the sum of two components: a user-specified fixed bias (OPBIAS.FIX) and a calculated floating bias (OPBIAS.FLOAT)
Real_A(OPEU,"in Engineering Units",8,0)
Boolean_A(OPEXHIFL,"OP High Limit Flag",8,false)//Indicates if OP has reached the high limit, as specified by OPHILM
Boolean_A(OPEXLOFL,"OP Low Limit Flag",8,false)//Indicates if OP has reached its low limit, as specified by OPLOLM
Boolean_A(OPHIFL,"OP High Limit Flag",8,false)//Indicates if OP has reached the high limit, as specified by OPHILM
Boolean_A(OPLOFL,"OP Low Limit Flag",8,false)//Indicates if OP has reached its low limit, as specified by OPLOLM
Real(OPREQ,"",0)
Boolean_A(OPROCNEGFL,"OP Negative Rate of Change Flag",8,false)
Boolean_A(OPROCPOSFL,"OP Positive Rate of Change Flag",8,false)
