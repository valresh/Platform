#include "StdAfx.h"
#include "../ARM.h"
#include "../AplicFrm.h"
#include "TagConCboModeAttr.h"

CTagConCboModeAttr::CTagConCboModeAttr(int n)
  {
  }
//
void CTagConCboModeAttr::Analiz( ANALIZ )
  {
  AnalizSPAN( this, html, teg );
  }
//
void CTagConCboModeAttr::Kerdyk( KERDYK )
  {
  CTag::Kerdyk( hWnd, html );
  //
  CStyle& style = Style(html);
  ASSERT( style.mClass == clsComboBox );
  if ( AS( mVal.def != NULL ) ) return; 
  ASSERT( mVal.def->eVal == enumValueChr );
  ASSERT( lstrcmpi( mVal.def->name, "MODEATTR" ) == 0 );
  //
  if ( style.m_eVisible == enumVisible )
    m_pWnd = new CHTML_ENUM( html, mVal );
  }
//
