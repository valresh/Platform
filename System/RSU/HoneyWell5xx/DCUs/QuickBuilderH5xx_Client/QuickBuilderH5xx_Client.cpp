...
//{
//  return new KQB_RemoteClient( (char*)ObjName);
//}

namespace rsu_cp {

	class RsuClientQuickBuilderH5xx : public RsuClient {
		KQB_RemoteClient* m_cl;
	...
	public:
		RsuClientQuickBuilderH5xx(const RsuSvrObjNames& a_names) : RsuClient(a_names)
		{
			std::string n = a_names.obj_name;
			m_cl = new KQB_RemoteClient((char*)(n.c_str()), this);
		}
		~RsuClientQuickBuilderH5xx()
		{
			delete m_cl;
		}
	};

	class RsuClientListQuickBuilderH5xx : public RsuClientList {
	protected:
		virtual std::unique_ptr<RsuClient> create_client(const RsuSvrObjNames& a_ObjNames)
		{
			return std::make_unique<RsuClientQuickBuilderH5xx>(a_ObjNames);
		}
	public:
		RsuClientListQuickBuilderH5xx(std::string a_svr_lib, std::string a_cfg_name, std::string a_svr_io, std::string a_ws, std::string a_rs, size_t a_svr_io_sz) :
	...
	typedef std::shared_ptr<RsuClientListQuickBuilderH5xx> PClients;
};
...
{
	static const char* g_svr_lib = "QuickBuilderH5xx_Server";
	...
	s_clients->push_back(std::make_shared<rsu_cp::RsuClientListQuickBuilderH5xx>(g_svr_lib, c,
