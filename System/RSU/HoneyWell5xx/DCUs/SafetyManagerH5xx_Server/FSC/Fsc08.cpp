#include "FscList.h"
static char* sid = "Delayed ON with variable timer value";
//
bool SFscList::FuncParser08( VAR_PARSER )
{
...
//
#ifdef _WIN32
void SFscList::FuncPaints08( VAR_PAINTS )
{
...
#endif
//
void SFscList::FuncSetLnk08( VAR_SETLNK )
{
  // Вход только справа
  if ( x == p.rect.right ) 
    return;
...
//  }
////-----------------------------------------------------------------------------
UINT* SFscList::FuncOutput08( VAR_OUTPUT )
{
  STrigger* dat = (STrigger*)fsc->Data(p.nBuff);
  n = 2;
  return &dat->Q;
...
////-----------------------------------------------------------------------------
void SFscList::FuncTimers08( VAR_TIMERS )
{
  if ( dt == 0  ) return;
  //
  STrigger* dat = (STrigger*)fsc->Data(p.nBuff);
...
int SFscList::FuncQuickWatch08( IFscStorage* fsc, LFscBase *obj, IFscStorage::SVarInfo *pvi, int cVI, LPCSTR *ppPntName )
{
...
int SFscList::SaveState08( IStateSer *psaver, IFscStorage* fsc, LFscBase *obj )
{
  STrigger* dat = (STrigger*)fsc->Data(obj->nBuff);
  SDotValue* vals[] = { fsc->DotV(dat->S), fsc->DotV(dat->R), fsc->DotV(dat->Q), fsc->DotV(dat->u), fsc->DotV(dat->X), fsc->DotV(dat->a) };
...
int SFscList::RestoreState08( IStateSer *prest, IFscStorage* fsc, LFscBase *obj )
{
  STrigger* dat = (STrigger*)fsc->Data(obj->nBuff);
  SDotValue* vals[] = { fsc->DotV(dat->S), fsc->DotV(dat->R), fsc->DotV(dat->Q), fsc->DotV(dat->u), fsc->DotV(dat->X), fsc->DotV(dat->a) };
  for( int i=0; i<_countof(vals); ++i )
