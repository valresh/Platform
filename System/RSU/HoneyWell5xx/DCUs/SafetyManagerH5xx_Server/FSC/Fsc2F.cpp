...
//
bool SFscList::FuncParser2F( VAR_PARSER )
{
  return LogicDB(fsc,p,dat,tmp);
}
//
//void FuncDebugs2F( VAR_DEBUGS )
//  {
//  strcat_s( str, sid );
//  strcat_s( str, "\r\n------------------------------------------------------------\r\n" );
//  FuncDebugsdo( p, str );
...
//
#ifdef _WIN32
void SFscList::FuncPaints2F( VAR_PAINTS )
{
  p.LogicOut( fsc, font, nSelect, "<" );
}
#endif
//
void SFscList::FuncSetLnk2F( VAR_SETLNK )
{
  // Вход только справа
  if ( x == p.rect.right ) 
    return;
...
//  }
////-----------------------------------------------------------------------------
UINT* SFscList::FuncOutput2F( VAR_OUTPUT )
{
  SLogicDB* dat = (SLogicDB*)fsc->Data(p.nBuff);
  n = 1;
  return &dat->O;
...
////-----------------------------------------------------------------------------
void SFscList::FuncTimers2F( VAR_TIMERS )
{
  SLogicDB* dat = (SLogicDB*)fsc->Data(p.nBuff);
...
int SFscList::FuncQuickWatch2F( IFscStorage* fsc, LFscBase *obj, IFscStorage::SVarInfo *pvi, int cVI, LPCSTR *ppPntName )
{
  return 0;
}

int SFscList::SaveState2F( IStateSer *psaver, IFscStorage* fsc, LFscBase *obj )
{
...
int SFscList::RestoreState2F( IStateSer *prest, IFscStorage* fsc, LFscBase *obj )
{
  SLogicDB* dat = (SLogicDB*)fsc->Data(obj->nBuff);
  SDotValue* vals[] = { fsc->DotV(dat->O) };
  for( int i=0; i<_countof(vals); ++i )
