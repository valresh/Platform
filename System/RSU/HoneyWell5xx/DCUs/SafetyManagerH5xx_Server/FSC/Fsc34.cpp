...
//
bool SFscList::FuncParser34( VAR_PARSER )
{
  return LogicDB(fsc,p,dat,tmp,enumValueDbl);
}
//
//void FuncDebugs34( VAR_DEBUGS )
//  {
//  strcat_s( str, sid );
//  strcat_s( str, "\r\n------------------------------------------------------------\r\n" );
//  FuncDebugsdo( p, str );
...
//
#ifdef _WIN32
void SFscList::FuncPaints34( VAR_PAINTS )
{
  p.LogicOut( fsc, font, nSelect, "/" );
}
#endif
//
void SFscList::FuncSetLnk34(VAR_SETLNK)
{
    // Вход только справа
    if (x == p.rect.right)
        return;
...
//  }
////-----------------------------------------------------------------------------
UINT* SFscList::FuncOutput34( VAR_OUTPUT )
{
  SLogicDB* dat = (SLogicDB*)fsc->Data(p.nBuff);
  n = 1;
  return &dat->O;
...
////-----------------------------------------------------------------------------
void SFscList::FuncTimers34( VAR_TIMERS )
{
  SLogicDB* dat = (SLogicDB*)fsc->Data(p.nBuff);
...
int SFscList::FuncQuickWatch34( IFscStorage* fsc, LFscBase *obj, IFscStorage::SVarInfo *pvi, int cVI, LPCSTR *ppPntName )
{
  return 0;
}

int SFscList::SaveState34( IStateSer *psaver, IFscStorage* fsc, LFscBase *obj )
{
...
int SFscList::RestoreState34( IStateSer *prest, IFscStorage* fsc, LFscBase *obj )
{
  SLogicDB* dat = (SLogicDB*)fsc->Data(obj->nBuff);
  SDotValue* vals[] = { fsc->DotV(dat->O) };
  for( int i=0; i<_countof(vals); ++i )
