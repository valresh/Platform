...
void KSMsPool::RegisterSM( KServer *pSM )
{
...
LPCSTR KSMsPool::GetName()
{
  return 0;
  //return ObjName;
}

bool KSMsPool::FindPoint( LPCSTR pszController, int nPlcAddress, SDotValue** ppV, void ***ppIO )
{
  for( int i=0; i<m_Count; ++i )
  {
    if( pszController && _strcmpi( m_Ctrls[i].pName, pszController ) )
      continue;
    else
      KKK();
    bool r = m_Ctrls[i].pSM->FindPoint( nPlcAddress, ppV, ppIO );
    if( pszController || r )
      return r;
  }
  return false;
