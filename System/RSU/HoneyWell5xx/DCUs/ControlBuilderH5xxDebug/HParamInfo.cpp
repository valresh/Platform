...
KHParamInfo::KHParamInfo(BYTE *pAddr, eVarType nType,  USHORT uStrLen, LPCSTR szEnumVals) : flags (0), VarName (NULL)
{
	Addr = pAddr;
	Type = nType;
	stringVarSize = uStrLen;
	pszEnumVals = szEnumVals;
}

void KHParamInfo::Init( BYTE *pAddObj, SVarInfo &var)
{
  if( !pAddObj )
    return;
  Addr = pAddObj + var.sVar;
  Type = var.Type;
  VarName = var.VarName;
  ASS( var.varSize < USHRT_MAX );
  stringVarSize = var.varSize;
  pszEnumVals = var.pszEnumVals;
}

void KHParamInfo::ManualInit( IControlBuilder *pcb, LPCSTR pBlock, LPCSTR pField )
{
  bool b = pcb->GetVar( pBlock, pField, &Addr, &Type, &stringVarSize, &pszEnumVals );
}
  ...
  case evtBool:
    {
      ...
      if( 1==v )
        ...
        sprintf_s( pszBuf, bufSize, "TRUE (%d)", v);
    }
    break;
  case evtByte:
    {
  ...
  case evtString:
    {
      ...
      pszBuf[0] = '"';
#ifdef _WIN32
      strncpy_s( pszBuf+1, bufSize-2, pv, _TRUNCATE );
#else
      strcpy_s((char*)(pszBuf + 1), (size_t)(bufSize - 2), (const char*)pv);
#endif
...
bool SetValue( LPCSTR pszVal, eVarType Type, BYTE *Addr, USHORT stringVarSize, LPCSTR pszEnumVals )
{
  int tmpForEnum = -100;
  if( pszEnumVals )
  {
    ...
    char szVals[4][256] = { 0 };
    const int C = SplitString( pszVal, szVals, ',', false );
    for( int n=0; n<C; ++n )
    {
    for( tEnumVals_::iterator it=Array.begin(), end(Array.end()); it!=end; ++it )
    {
        ...
        if( !_strcmpi( pU, szVals[n] ) || !_strcmpi( pU2, szVals[n] ) )
        {
  ...
  else
  {
    ...
    case evtDouble:
      if( IsFlt(pszVal) )
        ...
        *(double*)Addr = NaN;
      else if( !strcmp(pszVal,"+INF") )
        *(double*)Addr = std::numeric_limits<double>::infinity();
      else if( !strcmp(pszVal,"-INF") )
        *(double*)Addr = -std::numeric_limits<double>::infinity();
      else
      {
      ...
      break;
    case evtTime:
      *(time_t*)Addr = std::stoull(pszVal);
      break;
    case evtString:
      {
        size_t l = strlen( pszVal );
        ASSD( l < (stringVarSize+1) );
        if( l > stringVarSize )
          return false;
        strncpy_s( (char*)Addr, stringVarSize+1, pszVal, stringVarSize );
      }
      break;
    default:
      ASSD(0);
