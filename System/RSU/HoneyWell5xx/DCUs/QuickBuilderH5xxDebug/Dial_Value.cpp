...
void CDial_Value::SetVarList( int nCount, SQbParam *pVars )
{
  if( nCount <= 0 )
    return;
...
template<size_t _S>
void FillVal( char (&txt)[_S], SQbParam &var )
{
  switch( var.Type )
  {
  case 'D':
    if( _finite(*(double*)var.pAddr) )
      sprintf_s( txt, "%f", *(double*)var.pAddr );
    break;
  case 'I':
    sprintf_s( txt, "%d", *(int*)var.pAddr );
    break;
  case 'W':
    sprintf_s( txt, "%d", *(BYTE*)var.pAddr );
    break;
  case 'B':
    sprintf_s( txt, "%d", *(bool*)var.pAddr );
    break;
  case 'S':
    strcpy_s( txt, (LPCSTR)var.pAddr );
    break;
  case 'F':
    sprintf_s( txt, "%f", *(float*)var.pAddr );
    break;
  case 'H':
    sprintf_s( txt, "%d", *(short*)var.pAddr );
    break;
  default:
...
void CDial_Value::FillList()
{
  ...
  for( tVarsCont_::iterator it=m_Vars.begin(), end(m_Vars.end()); it!=end; ++it, ++p )
  {
    ...
    m_List.SetItemText( nItem, 1, var.ParamName+1 );
...
void SetValue( SQbParam &var, LPCSTR pszText )
{
  switch( var.Type )
  {
  case 'D':
    *(double*)var.pAddr = cross::locale::safe_atof(pszText);
    break;
  case 'I':
    *(int*)var.pAddr = atoi(pszText);
    break;
  case 'W':
    *(BYTE*)var.pAddr = atoi(pszText);
    break;
  case 'B':
    *(bool*)var.pAddr = atoi(pszText) ? true : false;
    break;
  case 'S':
    strcpy_s( (LPSTR)var.pAddr, var.Size, pszText );
    break;
  case 'F':
    *(float*)var.pAddr = (float)cross::locale::safe_atof(pszText);
    break;
  case 'H':
    *(short*)var.pAddr = atoi(pszText);
    break;
  default:
    ASS(0);
  }
...
void CDial_Value::OnEndLabelEdit(NMHDR* pNMHDR, LRESULT* pResult) 
{
  ...
  *pResult = 0;
  ...
  SetValue( var, pDispInfo->item.pszText );
}
