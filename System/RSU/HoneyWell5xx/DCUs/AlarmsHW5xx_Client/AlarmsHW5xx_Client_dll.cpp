...
namespace rsu_cp {
	...
	class RsuClientAlarmsHW5xx : public RsuClient {
		KAlarmClient* m_cl;
	...
	public:
		RsuClientAlarmsHW5xx(const RsuSvrObjNames& a_names) : RsuClient(a_names)
		{
			std::string n = a_names.obj_name;
			m_cl = new KAlarmClient((char*)(n.c_str()), this);
		}
		~RsuClientAlarmsHW5xx()
		{
			delete m_cl;
		}
	};

	class RsuClientListAlarmsHW5xx : public RsuClientList {
	protected:
		virtual std::unique_ptr<RsuClient> create_client(const RsuSvrObjNames& a_ObjNames)
		{
			return std::make_unique<RsuClientAlarmsHW5xx>(a_ObjNames);
		}
	public:
		RsuClientListAlarmsHW5xx(std::string a_svr_lib, std::string a_cfg_name, std::string a_svr_io, std::string a_ws, std::string a_rs, size_t a_svr_io_sz) :
	...
	typedef std::shared_ptr<RsuClientListAlarmsHW5xx> PClients;
};
...
{
	static const char* g_svr_lib = "AlarmsHW5xx_Server";
	...
	s_clients->push_back(std::make_shared<rsu_cp::RsuClientListAlarmsHW5xx>(g_svr_lib, c,
