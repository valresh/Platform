...
void SListValue::Attach(std::filesystem::path modulePath)
{
    ...
    //
    CharMP szBin;
    ::GetBinFile(szBin, modulePath.string().c_str(), "honeySMlist");
    //
...
#undef   SM_TYPE
#define  SM_TYPE( a, b, c ) { c, #b, id_##b, W_##b::BuildList },
...
SValueDef* NameToValueSM( DWORD eData, LPCTSTR name )
{
  return theList.Find( name, eData );
}

LPCSTR EnumStrSM( SValueDef& def, BYTE val )
{
  return theList.EnumStr( def, val );
}

BYTE EnumValSM( SValueDef& def, LPCSTR name )
{
