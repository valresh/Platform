...
{
  if ( !hSemValue->try_acquire_for(10000))
    return;
  
  CharMP szBin;
  ::GetBinFile(szBin, modulePath.string().c_str(), "honeyCBlist");
...
STagType* FindTagType( char* name )
{
  static STagType list[] =
  {
    ...
    { "OneMnemoHoney", "OneMnemoHoney", (EDataTypes)COneMnemoHoney::TypeID ,COneMnemoHoney::BuildList },
	{ "OneAlmGrHoney", "OneAlmGrHoney", (EDataTypes)COneAlmGrHoney::TypeID ,COneAlmGrHoney::BuildList },
#undef   HONEY_TYPE
#define  HONEY_TYPE( a, b, c ) { c, #b, id_##b, W_##b::BuildList },
...
int SortValueDef( const void * d1, const void * d2 ) 
{
  ...
  int ret = strcmpi( p1->name, p2->name );
  if ( ret == 0 )
  {
...
void SListValue::AddHWs( LPCSTR name, DWORD_PTR hw, DWORD_PTR sh, EValueType eType, UINT nType, UINT len )
{
  HNAME hName = mName.Add( name );
  SValueDef obj = { LPCSTR(hName), hw, sh, eType, 0, nType };
  obj.dwSize = len;
  AddObj( &obj );
}
...
UINT SListValue::AddEnum( const char* szEnum )
{
  ...
  while ( 1 )
  {
    ...
    if( !strchr(ptr,'=') )
    {
      HNAME hEnum = mName.Add( ('_'!=(*ptr)) ?  ptr : ptr+1 );
      mHand.AddObj( &hEnum );
    }
    //
    if( !p )
...
SValueDef* NameToValueH( DWORD eData, LPCTSTR name )
{
  return theList.Find( name, eData );
}

LPCSTR EnumStrH( SValueDef& def, BYTE val )
{
  return theList.EnumStr( def, val );
}

BYTE EnumValH( SValueDef& def, LPCSTR name )
{
