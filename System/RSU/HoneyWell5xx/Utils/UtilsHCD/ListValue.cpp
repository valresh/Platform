...
void SListValue::Attach(std::filesystem::path modulePath)
{
    ...
    //
    CharMP szBin;
    ::GetBinFile(szBin, modulePath.string().c_str(), "honeyHCDlist");
...
STagType* FindTagType( char* name )
{
  ...
  {
#undef   HCD_TYPE
#define  HCD_TYPE( a, b, c ) { c, #b, id_##b, W_##b::BuildList },
#include <HCDType.hpp>
  };
...
SValueDef* NameToValueHCD( DWORD eData, LPCTSTR name )
{
  return theList.Find( name, eData );
}

LPCSTR EnumStrHCD( SValueDef& def, BYTE val )
{
  return theList.EnumStr( def, val );
}

BYTE EnumValHCD( SValueDef& def, LPCSTR name )
{
