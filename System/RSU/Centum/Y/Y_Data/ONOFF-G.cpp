...
static CY_Create ONOFF_G( "ONOFF-G", CY_ONOFF_G::Create );

CY_ONOFF_G_W::CY_ONOFF_G_W()
{
  memset ( this, 0, sizeof(*this) );
}

CY_ONOFF_G::CY_ONOFF_G(CONOFF_G * _W ) : CY_Base()
{
  W = _W;
...
LIST_PARM(CY_ONOFF_G,CONOFF_G,33,53)

void CY_ONOFF_G::InitParm()
{
...
CSignal * CY_ONOFF_G::GetOUT ( char * Name )
{
  GET_STD_OUT
  GETSUB
  return NULL;
...
CSignal ** CY_ONOFF_G::GetIN ( char * Name )
{
  GET_IN(IN);
  GET_IN(SET);
  GET_IN(BIN);
...
bool CY_ONOFF_G::Double_OUT()
{
  ...
  if( pDI == NULL || !N )
    return false;
  CY_Base * pObj_1 = (*pFindDI)( pDR->NumbFCS, pDI->domain, N, pDI->Y_Type );
  ASSD( pObj && pObj_1 );
  if( pObj && pObj_1 )
  {
...
void CY_ONOFF_G::Work ( double dt )
{
  SET_HBP
  {
    KKK();
...
int CY_ONOFF_G::SaveState( )
{
  SAVE_OUT
  SUB.SaveState();
  SAVE_CLASS("W",CY_ONOFF_G_W)
  return 0;
}

int CY_ONOFF_G::RestoreState ( char * StrName )
{
  ...
  if ( SUB.RestoreState ( StrName ) == 0 )
    return 0;
  REST_CLASS("W",CY_ONOFF_G_W)
  return 1;
}

int CY_ONOFF_G::StateSave( IYokoStateSer* pStateSer )
{
...
int CY_ONOFF_G::StateRestore( IYokoStateSer* pStateSer )
{
...
int CY_ONOFF_G::GetParams( KYBridge2SysParam &params )
{
#include "IO_Parms.h"
