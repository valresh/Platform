...
namespace rsu_cp {
	...
	class RsuAlarmClientFlex : public RsuClient {
		KAlarmClient* m_flex_cl;		
	protected:
	...
	public:
		RsuAlarmClientFlex(const RsuSvrObjNames& a_names) : RsuClient(a_names)
		{
			std::string n = a_names.obj_name;
			m_flex_cl = new KAlarmClient((char*)(n.c_str()), this);
		}
		~RsuAlarmClientFlex()
		{
			delete m_flex_cl;
	...
	class RsuAlarmClientListFlex : public RsuClientList {
	protected:
		virtual std::unique_ptr<RsuClient> create_client(const RsuSvrObjNames& a_ObjNames)
		{
			return std::make_unique<RsuAlarmClientFlex>(a_ObjNames);
		}
	public:
		RsuAlarmClientListFlex(std::string a_svr_lib, std::string a_cfg_name, std::string a_svr_io, std::string a_ws, std::string a_rs, size_t a_svr_io_sz) :
	...
	typedef std::shared_ptr<RsuAlarmClientListFlex> PClients;
};
...
{
	static const char* g_svr_lib = "FlexRsuAlarms_Server";
	...
	s_clients->push_back(std::make_shared<rsu_cp::RsuAlarmClientListFlex>(g_svr_lib, c,
