...
bool EmfParser::load( struct SS * pSS, const QString & fileName )
{
  if ( pSS->NewPath )
    {
    pSS->data = NULL;
  ...
  if ( ! file->open( QIODevice::ReadOnly ) )
      ...
      pSS->Size = file->size();
      pSS->data = new char[pSS->Size];
      file->read(pSS->data,pSS->Size);
      file->close();
      delete file;
      }
    QByteArray Arr (pSS->data,pSS->Size);
  ...
  bool result = loadFromStream( pSS, stream );


	return result;
}

bool EmfParser::load(struct SS * pSS, const QByteArray &contents)
{
    ...
    return loadFromStream( pSS, emfStream);
}

bool EmfParser::loadFromStream(struct SS * pSS, QDataStream &stream)
{
