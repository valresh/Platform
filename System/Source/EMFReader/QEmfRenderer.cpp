...
//  return;
    ...
    QTransform tr = m_painter->combinedTransform();
	QRectF  rect(header->bounds());
 //   m_painter->setWindow( 0, 0, rect.right(), rect.bottom());
 //   m_painter->setViewport( 0, 0, rect.right(), rect.bottom());
    m_painter->save();
...
void QEmfRenderer::init( const Header *header, struct SS * pSS )
{
    ...
    if ( pSS )
    {
    m_painter->scale( pSS->ScaleX, pSS->ScaleY );
//    m_painter->translate(-header->bounds().left()+500., -header->bounds().top()+500.);//##
//    int x = header->bounds().left();
//    int y = header->bounds().top();//##
    m_painter->translate( pSS->ShiftX, pSS->ShiftY);//##
    }
    else
    {
...
void QEmfRenderer::eof()
  ...
  if ( pExtPaint )
    {
    (*pExtPaint) ( pSheme, m_painter );
      m_painter->restore();
    }
  //m_painter->fillRect( 100, 100, 100, 100, Qt::GlobalColor::white );
  //m_painter->drawText( QPointF(150,150), QString(" lhjq  fdjbscbMSBCjh2"));
...
void QEmfRenderer::extCreateFontIndirectW( const ExtCreateFontIndirectWRecord &extCreateFontIndirectW )
{
	...
	font.setWeight(static_cast<QFont::Weight>(convertFontWeight(extCreateFontIndirectW.weight())));
