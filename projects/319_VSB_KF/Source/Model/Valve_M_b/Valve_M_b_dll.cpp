...
extern "C" 
{
	EXP int Project = 1;
	EXP int Flags = 0;
	EXP char * Model =  "Valve_M_b";
	EXP char * Descr = "Description";
	EXP IBaseModel * CreateObject(LPCTSTR ObjName, LPCTSTR Type )
	{
		IBaseModel* pValve = NULL;
		LPCTSTR szType = Type;
		if(!lstrcmp(Type, "VEPC"))
			pValve = (IBaseModel*)new CVEP((char*)ObjName, (char*)Type);
		else if (!lstrcmp(Type, "VEPO"))
		{
			pValve = (IBaseModel*)new CVEP((char*)ObjName, (char*)Type);
		}
		else
			pValve = (IBaseModel*)new CV((char*)ObjName, (char*)Type);
		return pValve;
	}
};
...
STATIC int TestNodes( char * ObjName, int kNodes, struct CObjectPoint ** ppNodes )
{
	...
	return ValveStdTestNodes(ObjName, kNodes, ppNodes);
}

//MODEL_INFO(CValve_M_b, 0, "Valve_M_b", "Description")
