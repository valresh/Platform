...
function ResizeForTouchPanel()
{
    ...
    try
    {
        if(window.external.IsOnTouchPanel && !g_LoadedTouchLayout)
        {
            ...
            masterCSS.href = "Styles/EPKS_SummaryStyleStd_Touch.css";
            AreaTable.rowHeight = 35;
            Table.headerHeight = 35;
            Table.rowHeight = 35;
			g_lGridHeightAdjustment = g_lGridHeightAdjustmentOTP;
			HorizontalResizePad_BOTTOM.setAttribute("max", parseInt(HorizontalResizePad_BOTTOM.getAttribute("max")) - 35);
...
function ApplyThemes()
{
    ...
    if (themeCSS)
    {
        ...
        {
            case 2:
                themeCSS.href = "Styles/EPKS_SummaryStd_Light.css";
                break;
            case 1:
                themeCSS.href = "Styles/EPKS_SummaryStd_Dark.css";
                break;
            default:
                themeCSS.href = "Styles/EPKS_SummaryStd_Classic.css";
                try {
        ...
        if (!g_bInDegradedMode && !g_bWebCaptureMode)
        {
            ...
            if( null != v && DISPLAY_VERSION == v )
            {
                    ...
                    (g_layout_bag[PROP_Height] == g_lDisplayHeight))
                {
                    ...
                    for( i=0; i<g_arrPanesToPersist.length; i++ )
                    {
                        ...
                        else if (p.name == "AlarmTrendPane" && g_SummaryType == SUMMARY_ALARM)
                        {
                            if (g_layout_bag[p.name + "_DISPLAY"] != "none" && visible == "none") {
                                var previousDisplay = HPadAlarmTrend.style.display || "inline";
                                HPadAlarmTrend.style.display = "inline";
                                ShiftToolbar(AlarmTrendPane.offsetHeight + HPadAlarmTrend.offsetHeight);
                                HPadAlarmTrend.style.display = previousDisplay;
                            }
                            if (g_layout_bag[p.name + "_DISPLAY"] == "none" && GetURLParam("HscFile") != "0" && g_bAlarmTrendEnabled)
                    ...
                    if (AreaPane.offsetHeight > (50 * Table.rowHeight + Table.headerHeight))
                        AreaPane.style.height = PaneContainer.offsetHeight;
                    CheckGridSize();
        ...
        if (g_bPageInitialized && !g_bInDegradedMode && !g_bWebCaptureMode)
        {
            ...
            if (bSaveLayoutPersistence)
            {
                ...
                for( i=0; i<g_arrPanesToPersist.length; i++ )
                {
                    ...
                    {
                        var AlarmTrend = document.getElementById('alarmTracker');
                        if (AlarmTrend)
                        {
                            g_layout_bag[p.name + "_LabelsVisible"] = AlarmTrend.getAttribute("LabelsVisible");
                            g_layout_bag[p.name + "_TimePeriod"] = AlarmTrend.getAttribute("TimePeriod");
                        }
                    }
                }
                for( i=0; i<g_arrButtonsToPersist.length; i++ )
...
function OnOperatorChange()
{
    try
    {
        ...
        // Ensure alarm tracker is disabled if not licensed
        if (!g_bAlarmTrendEnabled)
        {
            OnAlarmTrendDisabledChange(!g_bAlarmTrendEnabled);
        }
...
{
	g_bAlarmTrendEnabled = !bAlarmTrendDisabled;
...
function OnNotifyErrorStatus(code, msg)
{
    ...
    else if( code == ERR_VIEW_DEGRADEDMODE )
    {
        ...
        if (!g_bInDegradedMode && ((g_SummaryType == SUMMARY_EVENT) || (g_SummaryType == SUMMARY_SOE) || (g_SummaryType == SUMMARY_EVENTVIEW)))
        {
            ...
            lblFilterApplied.style.visibility = "hidden";
    ...
    bPageAcknowledgeEnabled)
{
    ...
    if ((g_SummaryType == SUMMARY_ALARM) || (g_SummaryType == SUMMARY_MESSAGE) || (g_SummaryType == SUMMARY_SYSTEM_ALARM) || (g_SummaryType == SUMMARY_ALERT))
    {
        ...
        else
        {
            ...
            if (AlarmTrend)
            {
				securitySettingChangeEvent = document.createEvent('CustomEvent');
				securitySettingChangeEvent.initCustomEvent("securitySettingChanged", true, false, {
					ackEnabled: bPageAcknowledgeEnabled
				});
				document.dispatchEvent(securitySettingChangeEvent);
            }
        }
    }
...
{
    var PaneContainerHeight = Page.offsetHeight;
    var PaneContainerWidth = Page.offsetWidth;
    // Fix container overlapping alarm counts when in zoom-to-fit
    if(ParseZoom(Page.style.zoom) == 1.01) {
        PaneContainerHeight /= 1.01;
        PaneContainerWidth /= 1.01;
    }
    ...
    PaneContainerHeight -= paddingTop + paddingBottom;
    PaneContainerWidth -= paddingLeft + paddingRight;
    // Adjust the height of the Pane Container so that the last row will not be obscured
    PaneContainerHeight -= ((PaneContainerHeight - Table.headerHeight - 2) % Table.rowHeight);
...
function CheckGridSize()
{
    ...
    if (GridPane.offsetTop < 0)
        GridPane.style.top = "0px";
...
// --------------------------------------------------------------------------------------
function SetToolbar(element, propertyName)
{
    if( propertyName == "btnPrintAsReportDisabled" )
    {
    ...
    if( propertyName == "btnPrintPreviewDisabled" )
    {
    ...
    if( propertyName == "btnCsvExportDisabled" )
    {
...
function ToggleAlarmTrendPane()
{
    try
    {
        ...
        if (AlarmTrendPane.style.display == "none")
        {
			...
			alarmTrendPaneVisibilityEvent = document.createEvent('CustomEvent');
			alarmTrendPaneVisibilityEvent.initCustomEvent("alarmTrendVisibilityChanged", true, false, {
				visibility: true
			});
			document.dispatchEvent(alarmTrendPaneVisibilityEvent);
        }
        else
        {
            var heightChange = AlarmTrendPane.style.pixelHeight + HPadAlarmTrend.style.pixelHeight;
...
function HideAreaPane()
{
    ...
    {
        if(!AreaTableWrapper) {
            window.external.logMessage("AreaTableWrapper does not exist");
        } else {
            var rectAreaTable = AreaTableWrapper.getBoundingClientRect();
            if ((window.event.clientX >= rectAreaTable.left) &&
                (window.event.clientX <= (rectAreaTable.right + 100)) && //constant for natural left-right mouse movement
                (window.event.clientY >= rectAreaTable.top) &&
                (window.event.clientY <= rectAreaTable.bottom))
            return;
        }
    }
...
// --------------------------------------------------------------------------------------
function OnSelectionChange(DetailsXML, bSelectionChange, bSelectedRowDataChange, ulCookie)
{
    try
    {
        PopulateDetailsPane(DetailsXML, bSelectionChange, bSelectedRowDataChange, ulCookie);
    }
    catch( err )
    {
        HANDLE_ERROR("OnSelectionChange", err.number, err.description);
    }
}
// --------------------------------------------------------------------------------------
...
function ToggleAlarmHelpTab(bSelected)
{
    if (g_SummaryType == SUMMARY_ALARM || g_SummaryType == SUMMARY_SYSTEM_ALARM)
    {
        if (bSelected == true)
        {
            ...
            AlarmHelpData.style.display = "inline-block";
            RequestAlarmHelp();
        }
        else
        {
...
function ToggleBlockCommentsTab(bSelected)
{
    if(g_SummaryType == SUMMARY_ALARM)
    {
        if(bSelected == true)
        {
            ...
            BlockCommentsData.style.display = "inline-block";
        }
        else
        {
...
function ToggleGeneralTab(bSelected)
{
    if(bSelected == true)
    {
        ...
        SelectedRowData.style.display = "inline-block";
    }
    else
    {
...
function ToggleCommentsTab(bSelected)
{
    if((g_SummaryType == SUMMARY_EVENT) || (g_SummaryType == SUMMARY_ALARM) || (g_SummaryType == SUMMARY_SYSTEM_ALARM) || (g_SummaryType == SUMMARY_ALERT) || (g_SummaryType == SUMMARY_EVENTVIEW))
    {
        if(bSelected == true)
        {
            ...
            CommentsData.style.display = "inline-block";
            if ( AddComments.isContentEditable == true && AddComments.value != "" )
            {
                btnSaveDetails.setAttribute("readonly", false);
                btnSaveDetails.className = 'saveDetailsEnabled';
...
function FilterAppliedUpdate()
{
    ...
    {
        lblFilterApplied.style.visibility = "visible";
        if ((g_SummaryType == SUMMARY_ALARM) || (g_SummaryType == SUMMARY_MESSAGE) || (g_SummaryType == SUMMARY_SYSTEM_ALARM) || (g_SummaryType == SUMMARY_ALERT))
        {
            if (g_SummaryType != SUMMARY_MESSAGE)
            {
...
function OnViewSelect(elem)
{
	...
	var AlarmTrend = document.getElementById('alarmTracker');
    if (AlarmTrend) {
		clearFilterEvent = document.createEvent('CustomEvent');
		clearFilterEvent.initCustomEvent("clearFilter", true, false, {});    
...
function ZoomSummary()
{
    ...
    if (g_SummaryType == SUMMARY_MESSAGE)
        HorizontalResizePad_BOTTOM.setAttribute("max", g_lDisplayHeight - 450 - Table.rowHeight);
    else
        HorizontalResizePad_BOTTOM.setAttribute("max", g_lDisplayHeight - 350 - Table.rowHeight);
    if (g_SummaryType == SUMMARY_SYSTEM_ALARM)
...
function OnFilterChange()
{
    try
    {
        ...
        var AlarmTrend = document.getElementById('AlarmTrendPane');
        if (AlarmTrend)
        {
			...
			filterChangedEvent = document.createEvent('CustomEvent');
			filterChangedEvent.initCustomEvent("filterChange", true, false, {
				filter: strLocationFilter
			});
...
{
    var AlarmTrend = document.getElementById('AlarmTrendPane');
    if (AlarmTrend && getComputedStyle(AlarmTrend).visibility !== 'hidden')
    {
        if (cookie != 0)
        {
			alarmSummaryClickedEvent = document.createEvent('CustomEvent');
			alarmSummaryClickedEvent.initCustomEvent("alarmSummaryClicked", true, false, {
				alarmCookie: cookie
			});
			document.dispatchEvent(alarmSummaryClickedEvent);
        }
    }
}
function OnAlarmTrackerFocusOut()
{
	var AlarmTrend = document.getElementById('alarmTracker');
    if (AlarmTrend) {
		closeContextMenuEvent = document.createEvent('CustomEvent');
		closeContextMenuEvent.initCustomEvent("closeContextMenu", true, false, {});    
...
function UpdateCreateAndDataUI()
{
    ...
    if (creationUIElement != null) {
        if(ActCreateUI.style.display == "none" || DetailsPane.style.display == "none") {
            ...
            if (creationUIElement.show != null) {
                var entFilterString = divEntityFilter.filterString;
                creationUIElement.show(g_ActivityUIType, entFilterString ? entFilterString.split(";") : []);
            } else {
                window.external.logMessage("UpdateCreateAndDataUI(): creationUIElement not ready yet");
            }
        }
    }
    ...
    if (dataUIElement != null) {
        if(ActDataUI.style.display == "none" || DetailsPane.style.display == "none") {
            ...
            if (dataUIElement.show != null) {
                ...
                window.external.logMessage("UpdateCreateAndDataUI(): dataUIElement not ready yet");
            }
        }
    }
}
...
function RetrieveFiltersFromURL()
{
    try
    {
		...
		if(g_SummaryType == SUMMARY_MESSAGE)
		{
			...
			var rawSelectFirstRowArg = GetURLParam("SelectFirstRow");
			g_SelectFirstRow = !!decodeURI(rawSelectFirstRowArg);
			for( i=0; i<g_FilterURLParams.length; i++ )
			{
				var rawFilterArg = GetURLParam(g_FilterURLParams[i]);
				var rawFilterUrlArg = decodeURI(rawFilterArg);
				if (rawFilterUrlArg)
				{
					g_PersistanceSaveDisabled = true;
					g_FilterCols.push(g_FilterURLParams[i]);
					g_FilterVals.push(rawFilterUrlArg);
				}                
			}
		}
	}
...
function SetAckPageDisable()
{
    if (btnAckPage.btnAckPageDisabled)
    {
...
// --------------------------------------------------------------------------------------
function clickHDX(btn) {
    btn.selected = !btn.selected;
    if(btn.OnHDXClick) {
        btn.OnHDXClick.apply();
    }
}
// --------------------------------------------------------------------------------------
// ISR219404 - Workaround for IE DISPID mapping bug. Once the HMIWeb elements are injected into the DOM,
// the DISPID of the elements may change, causing property change notifications to be ignored or HSCStationWindow to crash.
// Workaround is to do a read of the property before DOM elements get injected, which for some reason
// locks the DISPIDs in place. Related ISR135891.
// https://support.microsoft.com/en-us/help/2855169/internet-explorer-may-not-return-the-same-dispid-for-the-same-property
// NOTE: the property have to be read in this specific way "element.myProperty" for the dispids to be locked.
// Reading the property like "element['myProperty']" will not lock the dispids.
function lockDispids() {
    var val;
    // lock dispids for comboboxes
    var comboBoxElements = document.getElementsByClassName("hsc.combo.1");
    for (var i = 0; i < comboBoxElements.length; i++) {
        var elem = comboBoxElements[i];
        val = elem.currentValue;
        val = elem.OptionList;
        val = elem.Message;
        val = elem.Quality;
        val = elem.QualityStatus;
    }
    // lock dispids for alphanumerics
    var alphaNumericElements = document.getElementsByClassName("hsc.alpha.1");
    for (var i = 0; i < alphaNumericElements.length; i++) {
        var elem = alphaNumericElements[i];
        val = elem.currentValue;
        val = elem.quality;
        val = elem.SecurityLevel;
        val = elem.Message;
        val = elem.QualityStatus;
        val = elem.NotifyAfterBlur;
    }
    // lock dispids for HVG
    var hvgElements = document.getElementsByClassName("hvg.base.1");
    for (var i = 0; i < hvgElements.length; i++) {
        var elem = hvgElements[i];
        val = elem.Message;
    }
    // lock dispids for scriptdata
    var scriptDataElements = document.getElementsByClassName("HSC.ScriptDataObject.1");
    for (var i = 0; i < scriptDataElements.length; i++) {
        var elem = scriptDataElements[i];
        val = elem.RuntimePointNames;
        val = elem.RuntimeParameters;
        val = elem.RuntimeValues;
        val = elem.Message;
    }
    // lock dispids for areaTable
    var areaTableElement = document.getElementById("AreaTable");
    if(areaTableElement) {
        var func = areaTableElement.OnInitialize;
        func = areaTableElement.OnRefresh;
        func = areaTableElement.OnAckTransaction;  
    }
}
document.addEventListener('DOMContentLoaded', lockDispids);
