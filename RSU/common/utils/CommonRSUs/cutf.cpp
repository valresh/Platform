...
// std::wstring Q_DECL_EXPORT s_2_ws(const std::string& s)
// {
//     std::wstring ret;
//     if (!s.empty())
//     {
...
{
    return utf8_towchar(s, SIZE_MAX, out, outsize);
}

size_t utf8zestimate(const char* s)
{
    return utf8_towchar(s, SIZE_MAX, NULL, 0);
}
...
{
    std::wstring ws;
    ws.resize(utf8_towchar(s, SIZE_MAX, nullptr, 0) - 1);
    utf8_towchar(s, SIZE_MAX, &ws[0], ws.length());
    return ws;
}
