...
bool is_valid_utf8( LPCSTR string )
{
  if(!string)
  {
    return true;
  }
  const BYTE * bytes = (const BYTE *)string;
  unsigned int cp;
  int num;
  while(*bytes != 0x00)
  {
...
int convert_utf8_to_windows1251( LPCSTR utf8, LPSTR windows1251, size_t n )
{
  size_t i = 0;
  ...
  for( ; j<n && utf8[i] && i<len; ++i )
  {
    ...
    else if ((~prefix) & 0x20) 
    {
      ...
      {
        for (int k = 0; k < _countof(g_letters); ++k) 
        {
...
LPCSTR Utf8ToAnsi( LPCSTR pszInUtf8 )
{
  if( !pszInUtf8 )
    return pszInUtf8;
  static char s_szBuf[1024] = {};
  Utf8ToAnsi( pszInUtf8, s_szBuf );
  return s_szBuf;
}
