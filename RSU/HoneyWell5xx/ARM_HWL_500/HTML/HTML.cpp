...
CHTML::CHTML(KHoneyPipeClient& _pipe)
    :
     pipe(_pipe),
//    ms_chVer(3)
    // //, mWnd( pWnd)
    ...
    , mPipes(sizeof(cross::SParamValueH))
    ,m_bPrint(false)
    , m_hHDC(NULL)
    , m_bR300(false)
    , m_bPopup(false)
...
bool CHTML::FindFile( const char* pszFolder, char* szPath )
{
    ...
    // if ( dwAttr & FILE_ATTRIBUTE_DIcross::RECTORY ) return false;
...
bool CHTML::FindName( char* szPath )
{
    ...
    if(!bFound)
    {
        // cross::CString filename(m_strFile);
        // cross::CString path(::GetBasePath());
        // path += "Abstract\\";
        // if(m_strFile.GetLength() >=_countof(".htm") && stricmp(m_strFile.GetBuffer() + (m_strFile.GetLength() - _countof(".htm") + 1), ".htm") != 0)
        //     filename += ".htm";
...
std::string CHTML::GetSafeviewCategory(const char* name)
{
    ...
    cross::CString strFile(name);
    if(stricmp(name + strlen(name)-_countof(".htm")+1, ".htm")==0)
        strFile = strFile.Mid(0, int(strlen(name)-_countof(".htm")+1));
    for(std::vector<std::string>::iterator it = m_ClietPath.begin(); it!=m_ClietPath.end(); it++)
    {
    //    sprintf_s( szPath, "%s%s\\%s.htm", ::GetBasePath(), (*it).c_str(), strFile.data() );
        DWORD dwAttr = GetFileAttributes( szPath );
        if ( dwAttr != -1 && !(dwAttr & FILE_ATTRIBUTE_DIRECTORY))
        { //нашли
            // char szDsdPath[MAX_PATH];
            //   sprintf_s( szDsdPath, "%s%s\\%s_files\\DS_datasource1.dsd", ::GetBasePath(), (*it).c_str(), strFile.data() );
            // DWORD dwSize;
...
//
bool CHTML::LoadHTM( LPCTSTR pszName, LPCTSTR szTagGrpIn )
{
    m_bFileLoaded = true;
    m_strFile  = pszName;
    char szTagGrp[1024] = "";
            ...
            if(Version == DOTF_PM)
            {
                ...
                if(Version == BERTH_ONPZ)
                {
                    if(m_strFile.CompareNoCase("sysdtldevctla_1_fp") == 0)
                    {
                        ...
                        {
                            DisplayDataRepositorySetValue("strCtlGOP", cross::CString(m_strObj+".GOP.PV").GetBuffer());
                            DisplayDataRepositorySetValue("strCtlGPV",  cross::CString(m_strObj+".GPV.PV").GetBuffer());
                            DisplayDataRepositorySetValue("strCtlOI",  cross::CString(m_strObj+".OI.PV").GetBuffer());
                            DisplayDataRepositorySetValue("strCtlPI",  cross::CString(m_strObj+".PI.PV").GetBuffer());
                            DisplayDataRepositorySetValue("strCtlSt0Cmd",  cross::CString(m_strObj+".{%BlkCMD4%}.PVFL").GetBuffer());
                            DisplayDataRepositorySetValue("strCtlSt1Cmd",  cross::CString(m_strObj+".{%BlkCMD5%}.PVFL").GetBuffer());
                            DisplayDataRepositorySetValue("strCtlSt2Cmd",  cross::CString(m_strObj+".{%BlkCMD6%}.PVFL").GetBuffer());
                            DisplayDataRepositorySetValue("strCtlLocalman",  cross::CString(m_strObj+".{%BlkLocalman%}.PV").GetBuffer());
                        }
                }
        }
    }
...
//    = ::FileToBuff( m_strFull );
 ...
 //        CHTML_Path path(m_strFull);
    ...
    //    {
    //      if(m_strFile.CollateNoCase("sncdtlSPCMa_fp")==0)
    //            if(m_strObj == cross::CString("FIC6112") || m_strObj == cross::CString("FIC7112"))
    //      {
...
//new implement for CSHARp
bool CHTML::LoadHTM(const cross::CString *str)
{
...
{
    static int nCount = 0;
    static cross::CString nameT[MAX_QUQU];
...
void CHTML::SetMnemoM(bool bWrite)
{
    ...
    for ( int n = 0; n < nCount; n++ )
    {
        ...
        str.Format( "%s;%s", list[n].name.c_str(), list[n].list[0] );
...
CHTML::CHTML(cross::CWnd* pWnd,KHoneyPipeClient& _pipe)
...
//
CTag* CHTML::PtInTag( cross::POINT& pt, bool bAll )
{
    if ( mBody == NULL ) return NULL;
    return mBody->PtInTag( *this, pt, bAll );
}
//
CTag* CHTML::PtInBoundTag(cross::POINT& pt )
{
    if ( mBody == NULL ) return NULL;
...
int CHTML::FindTags(cross::POINT pt, std::vector<CTag*>& tags)
{
    if ( mBody == NULL )
        return 0;
...
//
bool CHTML::FindTitle( cross::POINT& pt, LPSTR title )
{
    if ( mBody == NULL )
        return false;
...
//
CTag* CHTML::TabIndex( cross::POINT& pt )
{
    if ( mBody == NULL ) return NULL;
...
//
void CHTML::Dlg(class CDlgDebug& dlg, cross::POINT& pt, int nPaint )
{
    INT_PTR nCount = GetSize();
    CTag**     obj = GetData();
    // for ( INT_PTR n = 0; n < nCount; n++ )
...
void CHTML::ApplyLayout( float fScale, SIZE sz )
{
    ...
    //   pageS.mcross::RECT.Height = cy;
    //   backS.mcross::RECT.Width  = pageS.mcross::RECT.Width;
    //   backS.mcross::RECT.Height = pageS.mcross::RECT.Height;
    //   }
    // // Смешение элементов GrpLowerPanel
    // lower->ShiftDY( *this, dy );
    // // Изменение координат GrpCenterPanel
    // CStyle& style = centr->Style( *this );
    // float N = style.mcross::RECT.Height+dy;
    // if ( N < 100 )
    //   {
    //   centr->Disable( *this );
    //   if ( N < 0 )
    ...
    //     float s = N/style.mcross::RECT.Height;
    //     if(arrows =mBody->LinkScript( *this, "GRP_Arrows"))
    // 		arrows->ScaleDY( *this, style.mcross::RECT.Y, s );
    //     }
    ...
    //   //
    //   float s = N/style.mcross::RECT.Height;
    // centr->ScaleDY( *this, style.mcross::RECT.Y, s );
    //   //
    ...
    //     hiS.mcross::RECT.Y = taS.mcross::RECT.Y;
    //     loS.mcross::RECT.Height = (taS.mcross::RECT.Y+taS.mcross::RECT.Height)-loS.mcross::RECT.Y;
    //     loS.align |= DT_BOTTOM;
    //     }
    //   //CTag* tag = mBody->LinkScript( *this, "indicator_PV"  );
    //   }
            ...
            { //подпрограмма с параметрами
                ...
                if(pp4)
                {
                    ...
                    *pp4=0;
                    while(*tt1)
                    {
                        if(strncmp(tt1, "ByVal ", _countof("ByVal ")-1) == 0)
                        {
                            tt1 += _countof("ByVal ")-1;
                            SkipSpace(tt1);
                        }
                        char *tt2 = strchr(tt1, ',');
                        if(tt2)
...
{
    //  CStyle& style = Style( html );
    //  float shift = (style.mcross::RECT.Y-dy)*scale;
    //  style.mcross::RECT.Y = dy+shift;
    //  style.mcross::RECT.Height *= scale;
    //  //
    //  POINT* pt = style.PT(html);
    //  for ( int n = 0; n < style.m_nPointPT; n++ )
    //    {
...
CStyle &CHTML::Style(int n) {
    CStyle *list = (CStyle *)mStyle.m_szBuffer;
    return list[n];
}
