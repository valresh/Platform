...
extern "C" _EXP IBaseModel* CreateObject(LPCTSTR ObjName, rsu_cp::RsuClient* a_cl)
{
  return new KH_RemoteClient( (char*)ObjName, a_cl);
}

namespace rsu_cp {

	class RsuClientControlBuilderH5xx : public RsuClient {
		KH_RemoteClient* m_cl;
	...
	public:
		RsuClientControlBuilderH5xx(const RsuSvrObjNames& a_names) : RsuClient(a_names)
		{
			std::string n = a_names.obj_name;
			m_cl = new KH_RemoteClient((char*)(n.c_str()), this);
		}
		~RsuClientControlBuilderH5xx()
		{
			delete m_cl;
		}
	};

	class RsuClientListControlBuilderH5xx : public RsuClientList {
	protected:
		virtual std::unique_ptr<RsuClient> create_client(const RsuSvrObjNames& a_ObjNames)
		{
			return std::make_unique<RsuClientControlBuilderH5xx>(a_ObjNames);
		}
	public:
		RsuClientListControlBuilderH5xx(std::string a_svr_lib, std::string a_cfg_name, std::string a_svr_io, std::string a_ws, std::string a_rs, size_t a_svr_io_sz) :
	...
	typedef std::shared_ptr<RsuClientListControlBuilderH5xx> PClients;
};
...
{
	static const char* g_svr_lib = "ControlBuilderH5xx_Server";
	...
	s_clients->push_back(std::make_shared<rsu_cp::RsuClientListControlBuilderH5xx>(g_svr_lib, c,
