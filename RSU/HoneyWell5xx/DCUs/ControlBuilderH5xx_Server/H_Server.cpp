...
extern KNoName *g_Objs, *g_IOs;
//static KNoName *pMems[] = { g_Objs, g_IOs };
...
int KServer::AfterInit()
{
  if( m_bStep0 )
  {
    CBase* pBase;
    LPCSTR pszName = NULL;
    KNoName* pMems[] = { g_Objs, g_IOs };
    for( int g=0; g<_countof(pMems); ++g )
    {
      DWORD id = 0;
      while( pMems[g]->WhileBase( id, 0, &pBase, &pszName, NULL ) )
      {
...
int KServer::CalcStep( int dtMs )
{
  ...
  return KControlBuilder::StepTL( dtS );
}
...
{
  static LPCSTR s_szWndClassName = "HW5xxServer_ModelWindowClass";
...
LRESULT KServer::WndProcImpl( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
  ...
  case WM_USER+1:
    if( 1==wParam )
    {
      ...
      ShowACSObject( NULL, (LPSTR)pszName, NULL, false );
    }
    //else if( 2==wParam )
    //  KControlBuilder::InitL();
    break;
