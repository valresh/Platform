...
namespace rsu_cp {
	...
	class RsuClientFlex : public RsuClient {
		KClient* m_flex_cl;
	...
	protected:
		...
		{
			return m_flex_cl ? m_flex_cl->SetData(sd_SaveRSUState, path) : -1;
		}

		virtual int DoSaveParams(const char* path)
		{
			return m_flex_cl ? m_flex_cl->SetData(sd_SaveRSUParams, path) : -1;
		}
		
		virtual int DoRestoreState(const char* path)
		{
			return m_flex_cl ? m_flex_cl->SetData(sd_RestoreRSUState, path) : -1;
		}

		virtual int DoRestoreParams(const char* path)
		{
			return m_flex_cl ? m_flex_cl->SetData(sd_RestoreRSUParams, path) : -1;
		}
	...
	public:
		RsuClientFlex(const RsuSvrObjNames& a_names) : RsuClient(a_names) 
		{
			std::string n = a_names.obj_name;
			m_flex_cl = new KClient((char*)(n.c_str()), this);
		}
		~RsuClientFlex()
		{
			delete m_flex_cl;
	...
	class RsuClientListFlex : public RsuClientList {
	protected:
		virtual std::unique_ptr<RsuClient> create_client(const RsuSvrObjNames& a_ObjNames)
		{
			return std::make_unique<RsuClientFlex>(a_ObjNames);
		}
	public:
		RsuClientListFlex(std::string a_svr_lib, std::string a_cfg_name, std::string a_svr_io, std::string a_ws, std::string a_rs, size_t a_svr_io_sz) :
	...
	typedef std::shared_ptr<RsuClientListFlex> PClients;
};
...
{
	static const char* g_svr_lib = "FlexRsu_Server";
	...
	s_clients->push_back(std::make_shared<rsu_cp::RsuClientListFlex>(g_svr_lib, c,
