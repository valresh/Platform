...
CTreeListView::CMyTreeObj& CTreeListView::CMyTreeObj::operator=(CTreeListView::CMyTreeObj& obj)
{
...
int CTreeListView::OnCreateK(LPCREATESTRUCT lpCreateStruct, DWORD exStyle /*= 0*/ )
{
  ...
  dwStyle = WS_CHILD | WS_VISIBLE | TVS_DISABLEDRAGDROP | TVS_NOHSCROLL | TVS_HASLINES | TVS_LINESATROOT | TVS_HASBUTTONS | exStyle;
...
void CTreeListView::DrawTreeItem(bool bFocus, CRect rcItem, HDC hdc, HTREEITEM hItem)
{
  ...
  for(long i=0; i < m_nNrColumns; i++)
  {
    ...
    if( i>=m_Entries[hItem].m_strColumns.GetCount() )
      continue;
